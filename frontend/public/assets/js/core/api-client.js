...
// Custom helper to fetch JSON with automatic auth token and refresh handling
async function apiFetch(path, options = {}) {
  ...
  // If 401 Unauthorized, try to refresh the token using HTTP-only cookie
  if (response.status === 401 && path !== "/api/auth/login" && path !== "/api/auth/refresh") {
    ...
    try {
      ...
      if (refreshResponse.ok) {
        ...
        // Refresh token is also expired/invalid, sign out user
