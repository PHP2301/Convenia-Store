...
export default function Menu() {
  ...
  const filteredProducts = products.filter(product => {
    ...
    const matchesCategory = selectedCategory === 'all' || product.type.toLowerCase() === selectedCategory.toLowerCase()
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-6 border-b border-slate-200">
        <div>
          <h1 className="text-3xl font-black text-slate-800 uppercase tracking-wide">{t('thức ăn & thức uống')}</h1>
          <p className="text-slate-500 text-xs font-semibold uppercase mt-1">{t('ck go tự hào cung cấp các dịch vụ tiện ích đa dạng và chất lượng nhất')}</p>
        </div>
        ...
        <div className="flex items-center space-x-3 w-full md:w-auto">
          ...
          >
            ...
            <option value="nvt">Convenia Nguyễn Văn Thương (Phú Nhuận)</option>
      ...
      <div className="flex flex-col md:flex-row gap-5 justify-between items-center bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
        {/* Categories filters */}
        <div className="flex flex-wrap gap-2 w-full md:w-auto">
          {[
            { id: 'all', name: t('tất cả menu') },
            { id: 'food', name: t('thức ăn') },
            { id: 'drink', name: t('thức uống') }
          ].map((cat) => (
            <button
              key={cat.id}
              onClick={() => setSelectedCategory(cat.id)}
              className={`px-5 py-2.5 rounded-xl text-xs font-extrabold border transition-all duration-300 uppercase tracking-wider ${
                selectedCategory === cat.id
                  ? 'bg-[#00b4d8] border-transparent text-white shadow-md shadow-[#00b4d8]/20'
